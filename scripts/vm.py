...
print("\n")
subprocess.call('echo "deb http://http.kali.org/kali kali-rolling main non-free contrib" >> /etc/apt/sources.list', shell=True)
...
subprocess.call("apt remove crackmapexec -y", shell=True)
...
if os.path.isfile('/root/reverse.sh'):
    ...
    rev_ssh = '/usr/bin/autossh -M 0 -f -N -o "ServerAliveInterval 30" -o "ServerAliveCountMax 3" -R ' + rev_port + ':localhost:22 root@' + server
